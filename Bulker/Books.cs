...
namespace Bulker
{
    class BookUpdater : IQueryResult
    {
        ...
        private static string s_sBaseQuery =
            "SELECT TOP 1000 $$upc_books$$.ID, $$upc_books$$.Title, $$upc_books$$.Author, $$upc_books$$.Series, $$upc_books$$.Summary, $$upc_books$$.ReleaseDate, $$upc_books$$.UpdateStatus, $$upc_codes$$.LastScanDate, $$upc_codes$$.ScanCode FROM $$#upc_books$$";
        //                        0                    1                    2                     3                      4                       5                        6                              7                         8
        public void DoUpdate()
        {
            ...
            try
            {
                ...
                foreach (BookElementEx bex in m_plBooks)
                {
                    ...
                    if (Book.FScrapeBook(bex, out sError))
                    {
                        if (!DownloadCoverForBook(bex, m_config.LocalCoverRoot, "covers", out sError))
                        {
                            Console.WriteLine($"FAILED: to download cover art: {sError}");
                            LogBookUpdateError(bex, sError);
                        }
                        else
                        {
                            Console.WriteLine(
                                $"SUCCEEDED: Author: {bex.Author}, ReleaseDate: {bex.ReleaseDate}, Series: {bex.Series}");
                            AddBookToUpdateQueue(bex);
                        }
                    }
                    else
                    {
        ...
        void AddBookToUpdateQueue(BookElementEx bex)
        {
            StreamWriter sw = new StreamWriter(m_config.SqlFile, true /*fAppend*/, System.Text.Encoding.Default);

            UpdateStatus flagsToUpdate = UpdateStatus.Author | UpdateStatus.Series | UpdateStatus.ReleaseDate | UpdateStatus.CoverSrc | (UpdateStatus)bex.UpdateStatus;

            sw.WriteLine(BuildSetString(bex, null));
            sw.Close();
        }

        string BuildSetString(BookElementEx bex, string sComment)
        {
            List<string> plsSet = new List<string>();

            if (!String.IsNullOrEmpty(bex.ReleaseDate))
            {
            ...
            if (!String.IsNullOrEmpty(bex.Author))
                plsSet.Add($" Author='{Sql.Sqlify(bex.Author)}' ");

            if (!String.IsNullOrEmpty(bex.RawCoverUrl))
                plsSet.Add($" CoverSrc='{Sql.Sqlify(bex.RawCoverUrl)}' ");

            if (!String.IsNullOrEmpty(bex.Series))
                plsSet.Add($" Series='{Sql.Sqlify(bex.Series)}' ");

            UpdateStatus flagsToUpdate = UpdateStatus.Author | UpdateStatus.Series | UpdateStatus.ReleaseDate | UpdateStatus.CoverSrc |
                                         (UpdateStatus)bex.UpdateStatus;

            StringBuilder sb = new StringBuilder(256);
            sb.Append($"UPDATE upc_Books SET UpdateStatus={(int)flagsToUpdate} ");
        ...
        void LogBookUpdateError(BookElementEx bex, string sReason)
        {
            ...
            swSql.WriteLine(BuildSetString(bex, " -- FAILED QUERY!"));
